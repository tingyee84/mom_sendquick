...
$x = GetLanguage("send_sms",$lang);
$x1 = Getlanguage("get_contacts",$lang);
...
?>	
	...
	<?php if (isset($_GET["mobile_numb"])) { 
        $mobnum = urlencode($_GET["mobile_numb"]);
	// echo "$('#mobile').val('".urldecode($_GET["mobile_numb"])."')";
    echo "$('#mobile').val('".$mobnum."')";
// } else if (isset($_POST["msgid"])) { 
	...
	$.ajax({
		...
		url: "send_sms_lib.php",
		cache: false,
        data: "mode=getMessage&msgid={$_GET["msgid"]}", 
		dataType: 'json',
