...
$(document).ready(function() {
	$("#confirmpwd").on("keyup",function() {
		if($("#new_password").val() != $(this).val()) {
			$("#cfmpwdresult").html('<i class="fa fa-remove color-red"></i>');
		}else{
			$("#cfmpwdresult").html('<i class="fa fa-check color-green"></i>');
		}
	});
	$("#new_password").on("keyup",function() {
		if ($(this).val() != "") {
			if (pwdpatterncheck($(this).val())<2) {
				$("#pwdresult").html('<i class="fa fa-remove color-red"></i>');
				$("#new_password").get(0).setCustomValidity('Please follow the guideline on the right.');

			} else {
				$("#pwdresult").html('<i class="fa fa-check color-green"></i>');
				$("#new_password").get(0).setCustomValidity('');
			}
			$("#confirmpwd").keyup();
		} else {
			$("#pwdresult").html('<i class="fa fa-asterisk color-grey"></i>');
			$("#cfmpwdresult").html('<i class="fa fa-asterisk color-grey"></i>');
		}
	});
});
