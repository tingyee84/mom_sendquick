...
var table = $('#tbl_tmpl').DataTable({
	...
	processing: true,
	stateSave: true,
	ajax: {type:'POST',url:'message_template_lib.php',data:{mode:'listMessageTemplate'}},
	columnDefs: [{'targets': 2,'orderable': false}]
});
var date = $.now();
new $.fn.dataTable.Buttons(table, {
	// buttons: [
	// 	{extend:'csv',text:'<?php echo $xml_common->export.' CSV';?>',filename:'<?php echo $_SESSION['userid'];?>_TemplateMessage_'+date},
	// 	{extend:'excel',text:'<?php echo $xml_common->export.' Excel';?>',filename:'<?php echo $_SESSION['userid'];?>_TemplateMessage_'+date}
	// ]
	buttons: [
		{
			extend:'csv',
			text: '<i class="fa fa-file-text-o"></i> <?php echo $xml_common->export.' CSV'; ?>',
			filename:'<?php echo $_SESSION['userid'];?>_TemplateMessage_'+date,
...
$('#myCreate').on('show.bs.modal', function(e)
{
	...
	if(typeof id === "undefined") {
		...
		modal.find('#mode').val('addMessageTemplate');
	} else {
		modal.find('#header').html('<?php echo $x->edit_template;?>');
		modal.find('#mode').val('saveMessageTemplate');
		modal.find('#id').val(id);
		$.post('message_template_lib.php',{mode:'editMessageTemplate',id:id},function(val) {
	...
	if(confirm('<?php echo $x->alert_3; ?>')) {
		$('input[type=checkbox]').each(function() {     
			if (this.checked && this.value!='on') {
				$.post('message_template_lib.php',{mode:'deleteMessageTemplate',id:this.value},function() {
					table.ajax.reload();
				});
			}
		});
...
$('#truncate').on('click', function(e)
{
	if(confirm('<?php echo $x->alert_4; ?>')) {
		$.post('message_template_lib.php',{mode:'emptyMessageTemplate'},function() {
			table.ajax.reload();
		});
	}
});
...
$('#prevUpload').on('show.bs.modal', function(e)
{
	$('#upload_table').DataTable({
		...
		processing: true,
		autoWidth: false,
		ajax: {type:'POST',url:'message_template_lib.php',data:{mode:'listTemplate',access_type:'0'}}
	});
});
...
$('#preCancel').on('click', function(e){
	if(confirm('<?php echo $x->alert_6; ?>')) {
		$.post('message_template_lib.php',{mode:'deleteTemplate',access_type:'0'},function() {
